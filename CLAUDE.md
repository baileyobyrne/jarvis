...
- **Apple Reminders CalDAV VTODO permanently broken:** VTODO PUT requests return HTTP 200 and data is stored on iCloud CalDAV servers, but is completely invisible in the modern Reminders app (Apple migrated to CloudKit in iOS 13, 2019 — one-way, irreversible). Do NOT attempt Apple Reminders sync via CalDAV. Only VEVENT (calendar events) works. Alternatives: Make.com connector or `reminders-cli` via Mac companion.
- **iCloud CalDAV auth — Apple ID must be iCloud alias:** `ICLOUD_APPLE_ID` must be `bailey.obyrne@icloud.com` (iCloud alias), NOT `obyrnebailey@gmail.com` (Gmail). Wrong ID gives silent 401 on all CalDAV calls. Use app-specific password (not account password). Re-run `node scripts/icloud-setup.js` after any credential change to rediscover correct CalDAV URLs.
- **iCloud CalDAV:** `lib/ical-calendar.js` — exports `{ createCalendarEvent, fetchTodayEvents, createReminder, completeReminder, fetchReminders }`. `ICLOUD_REMINDERS_URL` also in `.env` (for VTODO CalDAV — non-functional for Apple Reminders, see gotcha above). `fetchTodayEvents()` returns `[]` silently if iCloud not configured. Setup/discovery: `node scripts/icloud-setup.js`.
...
- **`reminders` table (updated 2026-03-02):** `fire_at` nullable (tasks), `completed_at TEXT`, `is_task INTEGER DEFAULT 0`, `priority TEXT DEFAULT 'normal'`, `duration_minutes INTEGER DEFAULT 30`, `vtodo_uid TEXT` (CalDAV UID — populated but non-functional for Apple Reminders, see VTODO gotcha). Tasks skip iCal and Telegram dispatch.
...
- **PWA service worker cache versioning:** `workspace/dashboard/sw.js` — bump `const CACHE` whenever deploying changes to `dashboard.js` or `dashboard.css`. Current: `jarvis-v16`. Verify actual version with `head -1 workspace/dashboard/sw.js` — CLAUDE.md may lag MEMORY.md.
- **`RemindersPage` sub-components (dashboard.js):** `ReminderCard` (swipe right=complete, left=delete confirm, long-press=cycle priority), `SmartDatePicker` (date/time chips, replaces `datetime-local`), `RemindersStatsBar` (counts by bucket), `useSwipe` hook — all defined immediately before `RemindersPage`. Completion is optimistic with 3s undo toast before API call fires.
...
- **`GET /api/alerts` contact enrichment:** `outcomeMap` must store `{ outcome, called_at }` — NOT just `outcome`. `ContactCard.isCalled` is driven by `localCalledAt` (not `localOutcome`), so omitting `called_at` makes already-called contacts appear uncalled after a page reload.
...
- **`datetime` timezone:** Always use `datetime('now')` (UTC) — never `datetime('now','localtime')`. Schema defaults use UTC; mixing breaks chronological comparisons.
- **`willoughby-contacts.json` structure:** Top-level keys: `fetchedAt`, `total`, `lastPage`, `lastSuburbIdx`, `contacts`. The `contacts` field is an object keyed by numeric index strings (`"0"`, `"1"`, ...) — NOT an array. Total: 124,000 contacts.
...
- **`partners` + `referrals` tables (added 2026-03-01):** Referral business pipeline. `referrals.buyer_brief` is TEXT JSON blob — always parse with try/catch. `partners.fee_type`: `'percentage'` or `'flat'`.
- **`apiFetch` signature:** `apiFetch(path, token, options)` — token is always 2nd arg. Passing an options object as 2nd arg silently sends `Authorization: Bearer [object Object]` (all calls return 401).
...
- **Telegram fire-and-forget:** `setImmediate(async () => { try { await sendTelegramMessage(msg); } catch(e) { console.warn('...', e.message); } });` — never await in a request handler.
...
- **Buyer match alerts:** `notifyReferralBuyerMatches(event)` in snapshot-server.js — fires after every market event POST/PATCH, checks active buyer referrals for suburb match in `buyer_brief.suburbs`.
