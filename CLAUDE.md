...
- **PWA service worker cache versioning:** `workspace/dashboard/sw.js` has `const CACHE = 'jarvis-v7'`. Bump this string (e.g. `jarvis-v8`) whenever deploying changes to `dashboard.js` or `dashboard.css`, or installed PWA users will serve stale cached files. **Always verify current version in MEMORY.md — CLAUDE.md may lag.**
