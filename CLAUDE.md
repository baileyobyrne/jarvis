...
- **iCloud CalDAV:** `lib/ical-calendar.js` — `createCalendarEvent(opts)` helper. Configured via `ICLOUD_APPLE_ID`, `ICLOUD_APP_PASSWORD`, `ICLOUD_CALENDAR_URL` in `.env`. Setup/discovery: `node scripts/icloud-setup.js`.
