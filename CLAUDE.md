...
- **AgentBox buyer enquiries sync:** `node fetch-buyer-enquiries.js` pulls enquiries for all active McGrath Willoughby listings. Key discovery: REST API filter params (`filterlistingid` etc.) are ALL broken. Working approach: navigate admin master (`/mcgrathlovelocal/admin/master?iframe_in_react_app=1`) to capture `x-api-key` + `x-csrf-token` request headers, then use admin proxy API with **bracket notation**: `filter[listingId]=264P30930` — this actually filters. Two-step contact lookup: enquiry list → `/enquiries/{id}` (Bearer) → `attachedContact.id` → `/contacts/{id}` (Bearer). First sync: 353 buyers across 11 listings. Triggered from dashboard via POST `/api/buyers/sync`.
