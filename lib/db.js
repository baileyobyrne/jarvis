...
(function migrateRemindersDuration() {
  ...
  if (!cols.includes('vtodo_uid')) {
    db.prepare('ALTER TABLE reminders ADD COLUMN vtodo_uid TEXT').run();
    console.log('[db] reminders.vtodo_uid column added.');
  }
  if (!cols.includes('ios_synced_at')) {
    db.prepare('ALTER TABLE reminders ADD COLUMN ios_synced_at TEXT').run();
    console.log('[db] reminders.ios_synced_at column added.');
  }
})();
...
(function migrateRemindersOverhaul() {
  ...
  if (!cols.includes('priority')) {
    db.prepare("ALTER TABLE reminders ADD COLUMN priority TEXT DEFAULT 'normal'").run();
    console.log('[db] reminders.priority added');
  }
    ...
    db.transaction(() => {
      ...
      db.prepare(`INSERT INTO reminders_new
        SELECT id, contact_id, contact_name, contact_mobile, note, fire_at, sent, sent_at,
               created_at, duration_minutes, calendar_event_uid, NULL, NULL, 0, NULL
        FROM reminders`).run();
...
// ---------------------------------------------------------------------------
// Schema migrations — partners table (referral business)
// ---------------------------------------------------------------------------
(function migratePartnersTable() {
  const exists = db.prepare(
...
// ---------------------------------------------------------------------------
// Schema migrations — referrals table (referral business)
// ---------------------------------------------------------------------------
(function migrateReferralsTable() {
  const exists = db.prepare(
...
// ---------------------------------------------------------------------------
// Schema migrations — agentbox_contacts table (AgentBox contact mirror)
// ---------------------------------------------------------------------------
(function migrateAgentboxContactsTable() {
  const exists = db.prepare(
    "SELECT name FROM sqlite_master WHERE type='table' AND name='agentbox_contacts'"
  ).get();
  if (!exists) {
    db.prepare(`
      CREATE TABLE IF NOT EXISTS agentbox_contacts (
        id            TEXT PRIMARY KEY,
        name          TEXT,
        mobile        TEXT,
        email         TEXT,
...
// Performance indexes
// ---------------------------------------------------------------------------
try { db.prepare('CREATE INDEX IF NOT EXISTS idx_call_log_contact ON call_log(contact_id)').run(); } catch(_) {}
try { db.prepare('CREATE INDEX IF NOT EXISTS idx_call_log_called_at ON call_log(called_at)').run(); } catch(_) {}
try { db.prepare('CREATE INDEX IF NOT EXISTS idx_reminders_fire_at ON reminders(fire_at)').run(); } catch(_) {}
try { db.prepare('CREATE INDEX IF NOT EXISTS idx_market_events_detected ON market_events(detected_at)').run(); } catch(_) {}
try { db.prepare('CREATE INDEX IF NOT EXISTS idx_daily_plans_date ON daily_plans(plan_date)').run(); } catch(_) {}
