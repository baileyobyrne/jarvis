...
(function migrateRemindersOverhaul() {
  ...
  // Now check if fire_at is nullable by attempting a test insert
  // If it throws, we need to rebuild the table to relax the NOT NULL constraint
  try {
    const testId = db.prepare(
      "INSERT INTO reminders (contact_name, note, fire_at, is_task) VALUES ('__schema_test__', '__test__', NULL, 1)"
    ).run().lastInsertRowid;
    db.prepare('DELETE FROM reminders WHERE id = ?').run(testId);
    // fire_at is already nullable — no rebuild needed
  } catch (e) {
...
(function migrateBuyerProfiles() {
  ...
  if (!exists) {
    db.prepare(`CREATE TABLE buyer_profiles (
      id                INTEGER PRIMARY KEY AUTOINCREMENT,
      name              TEXT NOT NULL,
      mobile            TEXT,
      email             TEXT,
...
(function migrateBuyerMatches() {
  ...
  if (!exists) {
    db.prepare(`CREATE TABLE buyer_matches (
      id                  INTEGER PRIMARY KEY AUTOINCREMENT,
